...
\begin{equation}
    \Nx\cdot\TS{P}\trans+\BB{c}^0=\rho_0\ddot{\BB{y}}\text{, ou}
\end{equation}
...
%, cuja fronteira é $\Gamma_0=\partial\Omega_0$, $\BB{F}^a$ é uma força concentrada aplicada sobre um ponto $a$, cuja posição atual é $\BB{Y}^a$, $\BB{t}$ é uma força distribuída sobre uma superfície média da casca ($\BB{y}$) e $\rho_0$ é a massa específica inicial do contínuo.
...
\textcolor{red}{Indicar os eixos nas figuras, tanto do modelo estrutural como da malha.}
