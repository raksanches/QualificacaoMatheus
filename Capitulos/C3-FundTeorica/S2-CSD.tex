...
sendo possível definir um velor $\BB{g}$, denominado como vetor de desbalanceamento mecânico, como:
...
\begin{subequations}
    \begin{align}
         & (F^\mathrm{int})_i^a=\intDomi{\der{u_e}{Y_i^a}}=\intDomi{S_{kl}\der{\mathbb{E}_{kl}}{Y_i^a}}\text{,} &  & \text{Forças internas}          \\
         & (F^\mathrm{inerc})_i^a=\intDomi{\rho_0N_aN_b}\ddot{Y}_i^b\text{,}                                    &  & \text{Forças inerciais}         \\
         & (F^\mathrm{amort})_i^a=\intDomi{\lambda_m\rho_0N_aN_b}\dot{Y}_i^b\text{,}                            &  & \text{Forças de amortecimento}  \\
         & (F^\mathrm{c})_i^a=F_i^a+\intFronti{N_aN_b}T_i^a\text{ e}                                            &  & \text{Forças conservativas}     \\
         & (F^\mathrm{nc})_i^a=\intFronti{N_aN_b}Q_i^b\text{.}                                                  &  & \text{Forças não-conservativas}
    \end{align}
\end{subequations}
...
Isolando-se os termos incógnitos da aproximação de Newmark, substituindo-os em $\BB{g}$ e realizando as devidas simplificações, obtém-se:
...
\begin{subequations}
    \begin{align}
        &\der{A_{jk}}{Y_i^a}=(A^0_{lk})^{-1}N_{a,l}\dij\\
        &\Dder{\mathbb{E}_{kl}}{Y_i^a}{Y_j^b}=\frac{1}{2}\bigpar{\der{A_{ml}}{Y_i^a}\der{A_{mk}}{Y_j^b}+\der{A_{mk}}{Y_i^a}\der{A_{ml}}{Y_j^b}}\\
        &\der{S_{kl}}{Y_j^b}=\script{C}_{klmn}\der{\mathbb{E}_{mn}}{Y_j^b}\\
        &\der{\mathbb{E}_{kl}}{Y_i^a}=\frac{1}{2}\bigpar{A_{mk}\der{A_{ml}}{Y_i^a}+A_{ml}\der{A_{mk}}{Y_i^a}}\text{.}
    \end{align}
\end{subequations}
...
\textcolor{red}{DIZER QUE ESTÁ UTILIZANDO A QUADRATURA DE HAMMER.}

O algoritmo \ref{alg:teste} apresentado na forma de pseudocódigo exemplifica o código implementado.
