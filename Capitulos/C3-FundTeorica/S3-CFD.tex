...
Os escoamentos isotérmicos incompressíveis são governados pelos princípios da conservação da massa (Eq. \ref{eq:CFD-NS1}) e da conservação da quantidade de movimento (Eq. \ref{eq:CFD-NS2}), que na descrição ALE resultam em (ver anexo \ref{Ap:CFD}):
...
Sejam ainda os espaços das funções testes para a conservação de massa e conservação da quantidade de movimento $\script{V}_u$ e $\script{V}_p$, respectivamente, tais que:
...
Dessa forma, ponderando-se as equações \eqref{eq:CFD-NS1} e \eqref{eq:CFD-NS2} respectivamente por $\BB{w}$ e $q$, tal que $q \in  \script{V}_p$ e integrando-se sobre o domínio $\Omega$, obtém-se:
...
Como pode-se observar, a escolha dos espaços de aproximações que atendam às condições LBB não é trivial. Sendo assim, \citeonline{donea2003finite} apresenta alguns elementos finitos que atendem às condições LBB (denominados de Elementos Taylor-Hood), sendo alguns ilustrados na Figura \ref{fig:Taylor-Hood}.
...
Outra formulação que é eficiente tanto para a estabilização da convecção quanto do campo de pressão, englobando os mesmos termos da estabilização SUPG/PSPG e ainda introduzindo outros termos estabilizantes de forma consistente, é a formulação VMS.
