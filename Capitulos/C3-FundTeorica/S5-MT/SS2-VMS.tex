...
Inicia-se aqui com a apresentação da técnica aplicada às equações governantes na descrição Euleriana, com domínio fixo, para maior familiarização com o método, e na sequência apresenta-se formulação em descrição ALE \textcolor{red}{ACHO QUE NÃO FAZ DIFERENÇA INICIAR DIRETO NO ALE. ALÉM DISSO, \eqref{eq:WeakForm2} É ALE. APROVEITANDO, SEMPRE QUE ESTIVER SER REFERINDO A EQUAÇÕES, USE EQREF AO INVÉS DE REF.}, para domínios móveis. O sistema a ser resolvido parte do apresentado na Eq. \eqref{eq:NS-Euler}, que em sua forma fraca se encontra em \eqref{eq:WeakForm2}. Primeiramente realiza-se a separação dos membros em:
...
Seguindo \citeonline{Bazilevsetal.2007a} \textcolor{red}{VER REF. NO LIVRO DO TEZDUYAR} adota-se $w_i=\bar{w}_i$, $q=\bar{q}$ e assume-se que a parcela em escala fina $u'_i$ e $p'$ são modeladas como:
...
\begin{equation}
    ...
    \end{split}
\end{equation}
\textcolor{red}{Faça a substituição de $u'_j$ e $p'$. Se preferir pode apresentar a substituição em outra equação}.

Para a discretização do problema pode-se realizar a separação da dependência espacial e temporal para as funções tentativas e testes como:

\begin{subequations}
    \begin{align}
         & \bar{u}_i(\BB{y},t)=\sum_{\BB{\eta}^s}{U_i^a(t)N_a(\BB{y})}\text{,}\label{eq:uh}             \\
    ...
    \end{align}
\end{subequations}
\textcolor{red}{definir todas as variáveis da equação que ainda não foram definidos no texto (exemplo $\BB{\eta}^w$)}

Substituindo-se \eqref{eq:w-sep} e \eqref{eq:q-sep} em \eqref{eq:RBVMS1}, e levando-se em conta a arbitrariedade de $W^a$ e $Q^a$, a forma fraca espacialmente discreta fica \textcolor{red}{Procure outro símbolo para o resíduo ponderado, para não associal à função de forma. Não se preocupe em deixar diferente do livro...}:
...
\begin{subequations}
    ...
    \end{equation}
    \begin{equation}
        (N_\mathrm{C})^a=\intDom{N_a\bar{u}_{i,i}}-\SintDom{N_{a,i}u'_i}=0
    \end{equation}
...
Omitindo-se os dois últimos termos da equação \ref{eq:RBVMS1} e adotando-se valores diferentes de $\tau$ para as equações de conservação da quantidade de movimento e da continuidade, chega-se à formulação estabilizada SUPG/PSPG \cite{} \textcolor{red}{CITAR SUPG/PSPG}, dada por:
...
\noindent onde, seguindo  \citeonline{bazilevs2013computational}, adota-se $\tau_\pspg=\tau_\supg=\tau_\sups$ com sucesso para uma boa variedade de problemas.

Por sua vez, a formulação baseada em uma descrição ALE parte das equações apresentadas em \ref{eq:NS-ALE}, cuja forma fraca é dada por:
...
\noindent em que os parâmetros estabilizadores $\tau_\sups$ e $\nu_\lsic$ são alterados das equações \ref{eq:TermEstab}, onde se considera, ao invés da velocidade do fluido $\bar{u}_i$, a velocidade relativa à malha ($\bar{u}_i-\hat{u}_i$), ou seja, $\bar{u}_i\gets\bar{u}_i-\hat{u}_i$.

Para a representação do problema discreto, são utilizadas as mesmas aproximações das Eqs. \eqref{eq:uh}=\eqref{eq:q-sep}, porém as funções de forma $N_a(\BB{y},t)$ são agora definidas com base na configuração de referência da malha como:
...
Com isso, definem-se os vetores de resíduos ponderados da conservação de quantidade de movimento de da continuidade como:
...
Ainda é possível escrever, a partir da Equação \ref{eq:one-par-stable}, $\alpha_m$ e $\alpha_f$ em termos de um parâmetro arbitrário único  ($0\leq\rho_\infty\leq1$), que representa o raio espectral da matriz amplificação para $\Delta t\to\infty$, o qual é utilizado para controlar as dissipações de alta-frequência.
