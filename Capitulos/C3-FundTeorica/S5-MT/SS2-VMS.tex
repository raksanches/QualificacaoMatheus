...
\begin{algorithm}[h]
    ...
    \For{$t_i\gets 0$ \KwTo $t_f$}{
        ...
        \begin{flalign*}
             & \text{Vetor de acelerações nodais:}  &  & \BB{\dot{U}}_{n+1}^0\gets\frac{\gamma-1}{\gamma}\BB{\dot{U}}_n & \\
        ...
        \While{\textnormal{erro}>\textnormal{tol}}
        {
            \ForEach{\textnormal{elemento}}{
                \ForEach{\textnormal{ponto de Hammer }}{
                    ...
                    \begin{flalign*}
                         & \text{Vetor de posições nodais:}     &  & Y_{n+\alpha_f}^k      \gets \alpha_f Y_{n+1}^{k-1}+(1-\alpha_f)Y_n             & \\
                         & \text{Vetor de acelerações nodais:}  &  & \dot{U}_{n+\alpha_m}^k\gets \alpha_m \dot{U}_{n+1}^{k-1}+(1-\alpha_m)\dot{U}_n & \\
                         & \text{Vetor de velocidades nodais: } &  & U_{n+\alpha_f}^k      \gets \alpha_f U_{n+1}^{k-1}+(1-\alpha_f)U_n             & \\
                         & \text{Vetor de pressões nodais: }    &  & P_{n+1}^1             \gets P_n^{k-1}                                          &
                    \end{flalign*}\\
                    Cálculo dos termos estabilizadores $\tau_\sups$ e $\nu_\lsic$ \label{alg:estabilizador}\;
                    Cálculo das componentes da matriz tangente e dos vetores de resíduos\;
                }
            }
